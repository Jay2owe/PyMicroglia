...
Declare intervention and comparison windows, controls and independent samples. Keep conditional response models, observation-bounded timing, rhythm evidence and control contrasts separate.

The public action is `intervention_response`; its request declares `pipeline: "intervention-response"`.
...
Inspect `pymicroglia describe intervention_response` for current arguments. Request fields
