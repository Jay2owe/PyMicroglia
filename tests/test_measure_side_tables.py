...
Ported from Motion's ``analysis/test_side_tables.py`` on 2026-09-21 (stage 04 of the Motion port).
"""
...
from pymicroglia.measure.modules import load as _load_modules
_load_modules()
                         # collision check has the full vocabulary to check
                         # against rather than an empty one
from pymicroglia.measure.spec import MovieSpec as MovieConfig
from pymicroglia.measure.spec import SideTableSpec as SideTableConfig
from pymicroglia.measure.spec import _named_list
from pymicroglia.measure.inputs import load_side_table
from pymicroglia.measure import MeasurementContext, Scale
from pymicroglia.measure.summarise import join_side

def _side_tables(entries, resolve, stem):
    """Motion's ``config._side_tables``, as ``spec._named_list`` spells it now."""
    return _named_list(entries, "side table", stem,
                       lambda entry: SideTableConfig.from_dict(entry, resolve))
...
def test_the_columns_a_side_table_contributes_are_deliberately_undeclared():
    ...
    """
    from pymicroglia.measure.declare import declared_columns, list_derived, list_modules
...
def test_both_keys_reach_the_three_tables_a_reader_actually_opens(tmp_path):
    ...
    """
    from pymicroglia.measure.summarise import build_cell_summary, build_frame_summary
