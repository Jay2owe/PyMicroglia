...
def isolated(tmp_path, monkeypatch):
    ...
    return tmp_path
SMALL = {'t0': None, 't1': None, 'baselines': (6.0,), 'detrends': ('cubic',), 'ndecoy': 40, 'skip_videos': True, 'skip_control': True}
...
def test_a_confident_call_that_changes_nothing_is_a_note_not_a_question():
    ...
    notes = review.Review()
    notes.note('cosmic_rays', chosen=0.06, confidence='high', why="0.06 % of pixel-frames replaced, which is the reference dataset's own figure")
...
def test_low_confidence_opens_a_question_and_so_does_changing_a_result():
    notes = review.Review()
    notes.note('window', chosen='72-210 h', confidence='low', why='the record is broken by two gaps')
    notes.note('channels', chosen={'dluc': 0}, confidence='high', changes_result=True, why='every mask depends on this')
...
def test_the_report_leads_with_the_section_the_workflow_names(tmp_path):
    notes = review.Review()
    notes.flag('check', 'roi', 'The outline is automatic and unchecked', question='Does it follow the structure?')
...
def test_the_rebuild_is_much_smaller_than_the_engine_it_replaces():
    ...
    total = sum((len((folder / name).read_text(encoding='utf-8').splitlines()) for name in THE_REBUILD))
    assert total < 3643 // 2, f'what replaced dluc_pipeline.py is {total} lines against its 3,643; it should be a fraction of that, not most of it'

@pytest.fixture
def one_run(isolated):
    from pymicroglia.pipelines import dluc_single_cell
    source = oscillating_stack(isolated)
    manifest = dluc_single_cell.run(source, output_dir=isolated / 'out', if_exists='error', **SMALL)
...
def test_the_report_and_the_manifest_both_carry_the_review(one_run):
    ...
    assert '## What needs your eye' in (folder / 'REPORT.md').read_text(encoding='utf-8')
    assert manifest['review'], 'the manifest must carry the review'
    stored = read_document(folder / 'summary.json')
...
def test_the_open_questions_are_the_four_names_the_engine_uses(one_run):
    ...
    assert gates <= {'channels', 'window', 'roi', 'admissibility', 'cells', 'movement', 'candidate_geometry', 'control', 'cosmic_rays', 'registration', 'tissue_mask'}
...
def test_a_second_identical_run_refuses_rather_than_overwriting(one_run):
    ...
    with pytest.raises(FileExistsError):
        dluc_single_cell.run(source, output_dir=Path(manifest['folder']).parents[1], if_exists='error', **SMALL)

def test_skip_returns_the_previous_manifest_without_recomputing(one_run):
    from pymicroglia.pipelines import dluc_single_cell
    source, manifest = one_run
    again = dluc_single_cell.run(source, output_dir=Path(manifest['folder']).parents[1], if_exists='skip', **SMALL)
    assert again['reused'] is True
    assert again['run_label'] == manifest['run_label']
    assert again['summary']['off_tissue_sigma'] == manifest['summary']['off_tissue_sigma']

def test_a_changed_threshold_reuses_registration_and_cosmic_ray_removal(one_run):
    ...
    from pymicroglia.pipelines import dluc_single_cell
    source, manifest = one_run
    assert not any(manifest['summary']['cached'].values()), 'the first run built everything'
    again = dluc_single_cell.run(source, output_dir=Path(manifest['folder']).parents[1], if_exists='version', k_mask=7.0, **SMALL)
    cached = again['summary']['cached']
    assert cached['dluc_clean'] is True, 'cosmic-ray removal does not depend on a segmentation threshold'
    assert all((value for name, value in cached.items() if name.startswith('registered_'))), 'registration does not depend on a segmentation threshold either'

def test_a_changed_cosmic_threshold_reuses_registration_but_not_the_cleaning(one_run):
    """The other direction, which is what makes the first test mean something."""
    from pymicroglia.pipelines import dluc_single_cell
    source, manifest = one_run
    again = dluc_single_cell.run(source, output_dir=Path(manifest['folder']).parents[1], if_exists='version', cosmic_seed_z=10.0, **SMALL)
    cached = again['summary']['cached']
    assert cached['dluc_clean'] is False
    assert all((value for name, value in cached.items() if name.startswith('registered_')))
...
def test_answering_a_question_stops_it_being_asked_again(one_run, isolated):
    ...
    again = dluc_single_cell.run(source, output_dir=Path(manifest['folder']).parents[1], if_exists='version', **SMALL)
    assert 'channels' not in {item['gate'] for item in again['open_questions']}
    ...
    try:
        ...
        after = dluc_single_cell.run(source, output_dir=Path(manifest['folder']).parents[1], if_exists='version', **SMALL)
        assert 'channels' not in {item['gate'] for item in after['open_questions']}
    finally:
...
def test_the_pipelines_available_are_exactly_the_ones_named_and_no_others():
    ...
    assert pipelines.PIPELINE_NAMES == ('auto_microglia', 'dluc_single_cell', 'cry1_dluc_photon', 'bioluminescence', 'phase_green_red', 'rhythm_discovery', 'method_audit', 'measurement_relationships', 'behaviour_states', 'spatial_coordination', 'intervention_response')
...
def test_the_two_registered_protocols_are_no_longer_pending():
    ...
    """
    import pymicroglia.registry as registry
    ...
    assert registry.REGISTRY.binds_to('dluc_single_cell') == 'pipelines.dluc_single_cell.run'
    assert registry.REGISTRY.binds_to('cry1_dluc_photon') == 'pipelines.cry1_dluc_photon.run'
...
def test_the_photon_pipeline_reproduces_its_engine(isolated):
    ...
    manifest = cry1_dluc_photon.run(CRY1_SOURCE, output_dir=isolated / 'cry1', if_exists='overwrite', videos=False)
    mine = manifest['summary']
    theirs = read_document(CRY1_ENGINE / 'processing_manifest.json')
    for key in ('frames', 'crop_xyxy', 'output_shape', 'recorded_median_interval_seconds', 'registration_residual_median_px', 'registration_residual_p95_px', 'registration_residual_max_px'):
    ...
    for column in ('pair_shift_y_px', 'pair_shift_x_px', 'cumulative_shift_y_px', 'cumulative_shift_x_px', 'residual_magnitude_px'):
        np.testing.assert_array_equal([float(row[column]) for row in ours], [float(row[column]) for row in engine], err_msg=column)
    ours = _csv_rows(folder / 'photon_signal_qc.csv')
    engine = _csv_rows(CRY1_ENGINE / 'photon_signal_qc.csv')
    for mine_column, their_column in (('camera_baseline', 'camera_baseline_p30'), ('isolated_event_threshold_counts', 'isolated_event_threshold_counts'), ('isolated_event_pixels', 'isolated_event_pixels'), ('positive_signal_sum', 'positive_signal_sum'), ('positive_signal_percentile', 'positive_signal_p99')):
        np.testing.assert_array_equal([float(row[mine_column]) for row in ours], [float(row[their_column]) for row in engine], err_msg=mine_column)
    stem = CRY1_SOURCE.stem
    engine_stack = tifffile.imread(CRY1_ENGINE / f'{stem}_registered_raw_4channel.tif')
...
def test_the_general_pipeline_runs_the_live_cosmic_method(isolated):
    ...
    manifest = bioluminescence.run(source, output_dir=isolated / 'out', segment=False, test_rhythm=False, claim='the standard order, on a synthetic stack')
    order = [entry['stage'] for entry in manifest['stages']]
    pipelines.check_stage_order(order)
    assert order.index('register') < order.index('cosmic_rays')
...
def test_the_general_pipeline_flags_a_cut_that_rewrites_too_much(isolated):
    ...
    from pymicroglia.pipelines import bioluminescence
    source = oscillating_stack(isolated, frames=12)
    manifest = bioluminescence.run(source, output_dir=isolated / 'out', segment=False, test_rhythm=False, seed_z=2.5, claim='what a cut this low does to the stack')
    flagged = [item for item in manifest['review'] if item['gate'] == 'cosmic_rays']
    assert flagged, 'a filter rewriting most of the stack should be flagged'
    assert any(('raise seed_z' in str(item.get('remedy', '')) for item in flagged))
