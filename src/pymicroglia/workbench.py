...
**Temporary.** A straight copy of Motion's ``analysis/circadian.py`` made on
2026-09-21 as stage 04 of the Motion port, so that the ``rhythms``, ``coupling``
and ``recurrence`` modules reach Circadian Workbench through exactly one
module of this package. Stage 05 collapses it onto the Workbench's public
facade; until then this is the only file under ``pymicroglia`` that imports
``circadian_workbench``.
"""
...
def normalize_trace(
    ...
    *,
    ...
    caller = cw.trace(hours, values, name="motion trace")
    # Workbench processing reports hours from its selected recording start and
    # carries the corresponding clock origin in ProcessedTraceData. This facade
    # accepts recording coordinates supplied by Motion, including late-starting
    # cells. Translate reference windows explicitly, then verify the public
    # processed clock before pairing transformed values with those coordinates.
    original_hours = np.asarray(caller.trace_data.hours, dtype=float)
    offset = float(original_hours[0])
    result = caller.normalize(
        method=method,
        target_min=target_min,
        target_max=target_max,
        reference_value=reference_value,
        reference_start_hours=None if reference_start_hours is None else reference_start_hours - offset,
        reference_end_hours=None if reference_end_hours is None else reference_end_hours - offset,
    ...
    if len(original_clock) != len(processed_clock) or not np.allclose(
            (processed_clock.to_numpy() - original_clock.to_numpy()) / np.timedelta64(1, "h"),
            0., rtol=0., atol=1e-9):
    ...
    data["processed_trace"] = _processed_trace_in_input_time(caller, result)
...
def detrend_trace(hours: Sequence[float], values: Sequence[float], params: dict,
    ...
    caller = cw.trace(hours, values)
    completed = caller.detrend(
    ...
    data["workbench_processed_trace"] = deepcopy(data["processed_trace"])
    data["processed_trace"] = _processed_trace_in_input_time(caller, completed)
...
def estimate_one(hours: Sequence[float], values: Sequence[float], params: dict,
                 ...
                 detrend_window_hours: float | None = None,
                 capture_details: bool = False) -> dict[str, Any]:
    ...
    if capture_details:
        # Keep the native completed payload, not a second fit reconstructed from
        # its headline period. Methods do not all export a waveform or trace.
        ...
        result["effective_settings"] = deepcopy(
            completed.run_record.get("configurations", []))
        result["phase_reference"] = returned.get("phase_reference")
        result["phase_units"] = returned.get("phase_units")
        # Period actions do not currently export their internal prepared frame.
        # Save a separately identified public detrending result for inspection,
        # without repeating the estimator or claiming an internal used-row mask.
        result["display_processed_trace"] = None
        result["display_processing_reason"] = "method handles its own detrending internally"
        if not PERIOD_METHODS[method].get("forced_detrending"):
            try:
                processing = trace.detrend(
                    method=config["period_detrend"], settings=config,
                    **{name: config["period_detrend_" + name] for name in (
                        "window_hours", "polynomial_degree", "min_valid_fraction", "bandwidth_hours",
                        "low_cut_hours", "high_cut_hours", "filter_order", "lowess_fraction",
                        "lowess_iterations", "asls_smoothness", "asls_asymmetry", "asls_iterations")})
                result["display_processed_trace"] = _processed_trace_in_input_time(trace, processing)
                result["display_processing_run_record"] = deepcopy(processing.run_record)
                result["display_processing_reason"] = (
                    "Separate Workbench detrending diagnostic with the applied configuration; "
                    "not an exported internal estimator frame; public clock expressed in input recording hours")
            except (cw.WorkbenchError, ValueError) as error:
...
def estimate_grouped_rhythms(
    ...
    *,
    ...
    for key, group in frame.groupby(grouper, sort=True, dropna=False):
        ...
        usable = usable[np.isfinite(usable[time_column].to_numpy(float))
                        & np.isfinite(usable[value_column].to_numpy(float))]
        ...
        except (ValueError, RuntimeError, cw.WorkbenchError) as error:
            estimate = {
                "status": "failed",
        ...
        rows.append({
            **base,
            **({"estimate_result": deepcopy(estimate),
                "significance_result": deepcopy(evidence)} if capture_details else {}),
