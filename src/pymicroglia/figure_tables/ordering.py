...
def ordered_cell_keys(summary: pd.DataFrame, order_by: Sequence[str] = ()) -> list[tuple]:
    """Stable ordering with the complete source-run/movie/cell identity intact."""
    keys = ["source_run", "movie", "identity"]
    if any(key not in summary for key in keys):
        raise ValueError("Cell ordering requires source run, movie and local identity")
    if summary.duplicated(keys).any():
        raise ValueError("Cell ordering contains duplicate full identities")
    sortable = summary.copy()
    sortable["_cell_key"] = list(map(tuple, sortable[keys].to_numpy()))
    order = resolve_order(order_by, append=keys, identity_column=None)
    return ordered_identities(sortable, order, identity_column="_cell_key")
