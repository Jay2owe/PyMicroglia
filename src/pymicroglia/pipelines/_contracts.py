...
def result_to_dict(result: StepResult) -> dict:
    """Encode selection members with explicit types for other pipeline recipes."""
    data = result.as_dict()
    for selection, encoded in zip(result.selections, data["selections"]):
        encoded["members"] = [
            {"type": type(member).__name__, "key": plain(member)}
            for member in selection.members]
    return data


def result_from_dict(data: dict) -> StepResult:
    """Read the runner's typed outcome; do not guess keys from their shape."""
    constructors = {
        "CellKey": lambda key: CellKey(**key),
        "CellMeasurementKey": lambda key: CellMeasurementKey(CellKey(**key["cell"]), key["measurement"]),
        "MeasurementPair": lambda key: MeasurementPair(**key),
        "Settings": Settings,
    }
    refs = tuple(ArtifactRef(**{**ref, "grain": tuple(ref["grain"]),
                               "columns": tuple(ref["columns"])}) for ref in data["artifacts"])
    selections = []
    for selection in data["selections"]:
        members = []
        for member in selection["members"]:
            if member["type"] not in constructors:
                raise ValueError(f"unknown selection member type: {member['type']}")
            members.append(constructors[member["type"]](member["key"]))
        selections.append(SelectionRecord(selection["name"], selection["scientific_id"],
                                          Settings(selection["rule"]), tuple(members)))
    return StepResult(data["step"], data["scientific_id"], data["status"], data["reason"],
                      refs, tuple(selections), Settings(data["provenance"]))
