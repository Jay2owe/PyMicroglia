...
substring match is how a renamed column silently leaves the set with nothing
failing. A group is a name, an explicit membership, and a check that every
member is a column some module says it writes.

Two ways to declare one::

    "metric_groups": {
      "circadian": ["cosinor_amplitude", "relative_amplitude"],
      "drift":     {"module": "trend"}
    }

The list is taken literally. The selector block reads what the modules already
declare -- ``role``, ``module``, or both, which are ANDed.

Anywhere a list of columns is expected, ``"@circadian"`` stands for the group's
members. One level only: a group may not be built from another group.

Ported from Motion's ``analysis/metric_groups.py`` on 2026-09-21. This module
never imports a figure; a run that only measures must stay possible in an
environment with no matplotlib.
"""
...
from .declare import Column, declared_columns, list_derived, list_modules
...
class MetricGroup:
    ...
    Both halves are kept. The declaration is what the author wrote; the
    resolved columns are what was actually used, and are what a run records --
    a selector group means something different after a module gains a column.
    """
...
def resolve_metrics(values, groups: dict[str, MetricGroup], *,
    ...
    position. ``where`` names the setting in any error.
    """
...
def resolve_setting(value, groups: dict[str, MetricGroup], *, where: str):
    ...
    comes back exactly as written, ``"a,b"`` included.
    """
