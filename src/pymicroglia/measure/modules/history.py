...
Ported from Motion's ``analysis/modules/history.py`` on 2026-09-21 (stage 04 of the
Motion port); the arithmetic is unchanged. What changed is where the list of
decision tables comes from: Motion kept fourteen paths as constants here, and
this reads them from the ``DecisionTables`` the tracking result declares.
"""
...
from pymicroglia.tracking.contract import (MOTION_DECISION_TABLES,
                                           MOTION_LABEL_TABLES, DecisionTables)

#: Bumped when this module's arithmetic changes; recorded in the run record.
METHOD_VERSION = "2026-09-21-history-v1"

#: The decision tables this module knows how to copy through are declared on
#: the tracking result, not here. ``DecisionTables`` (stage 02 of the Motion
#: port) carries ``(name, relative path, grain)`` for every CSV a tracker
#: emits, and a movie's tables arrive on ``context.decisions``. What follows
#: are the *names* today's Motion tracker declares, read from the same
#: contract so the ``Output`` declarations below cannot drift from it: the
#: fourteen under the decision folder and the two written beside the labels.
#: The pixel-evidence table is deliberately absent from both: it is one row
#: per renamed pixel and belongs in the tracking run, not in every analysis
#: run.
DECISION_NAMES: tuple[str, ...] = tuple(name for name, _, _ in MOTION_DECISION_TABLES)
LABEL_NAMES: tuple[str, ...] = tuple(name for name, _, _ in MOTION_LABEL_TABLES)


def _decisions(context: MeasurementContext) -> tuple[DecisionTables | None, Path | None]:
    """The tracker's declaration for this movie, and where the labels sit.

    ``context.decisions`` is what the loader put there from the tracking
    result. A caller that built a context by hand may instead name the
    decision folder as the module's ``directory`` setting, in which case
    today's Motion declaration is assumed for it; that is the older route and
    it is kept so a configuration file that says ``history`` still works.
    """
    params = context.module_params("history")
    tables = getattr(context, "decisions", None)
    if tables is None:
        directory = params.get("directory")
        if directory and Path(directory).is_dir():
            tables = DecisionTables.motion(directory)
    labels_root = params.get("labels_directory")
    labels_root = Path(labels_root) if labels_root and Path(labels_root).is_dir() else None
    return tables, labels_root


def _read_sources(root: Path | None, sources: tuple[tuple[str, str, Any], ...],
                  where: str) -> tuple[dict[str, pd.DataFrame], list[dict]]:
    found: dict[str, pd.DataFrame] = {}
    record: list[dict] = []
    for name, relative, _grain in sources:
...
#: tracking contract's names above so the declaration cannot drift from what
#: is read.
#:
...
WRITES = tuple(
    ...
    for name in (*DECISION_NAMES, *LABEL_NAMES)
) + (
...
def derive(cell_frame: pd.DataFrame, context: MeasurementContext) -> dict[str, pd.DataFrame]:
    declared_tables, labels_root = _decisions(context)
    if declared_tables is None:
        # No tracker declaration for this movie: run.py records the skip;
        # there is nothing to say and nothing to hide.
        return {}
    root = Path(declared_tables.root)
    found, record = _read_sources(root if root.is_dir() else None,
                                  declared_tables.tables, "decision folder")
    beside, beside_record = _read_sources(labels_root, declared_tables.beside,
                                          "beside the labels")
    found.update(beside)
