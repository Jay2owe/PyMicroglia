...
1. **Two patterns matching one stem is an error, not a race.**
2. **No pattern matching a stem leaves it unassigned**, and the run refuses to
   start.
3. **Nothing is matched until you have looked at it**: the assignment table is
   in the run manifest.

Crossed designs come free. Give each condition a ``factor`` and every factor is
resolved independently, then combined: a stem matching ``_A2`` and ``18m``
becomes ``HCQ_old``.

**Colours.** Which colour a genotype should be is a decision about the science,
so a project names its own. This module records the colour a condition was
given -- a ``#rrggbb`` literal or a palette name such as ``okabe_blue`` -- and
resolves nothing to a hexadecimal value itself: the palette lives with the
theme (a later port stage), and a chassis that carried its own copy would be
one more table to drift. Motion resolved names to hex here; the names it
accepted are exactly those passed through unchanged.

Ported from Motion's ``analysis/conditions.py`` on 2026-09-21.
"""
...
#: one for. Okabe-Ito palette names, because this is the one case where the
#: choice is arbitrary and so should at least stay legible to a colourblind
#: reader. Names, not values: the theme resolves them.
...
_PALETTE_NAME = re.compile(r"^[a-z][a-z0-9_]*$")


# ----------------------------------------------------------------- colours

def resolve_colour(value: Any, name: str) -> str:
    """One colour spec checked and normalised, for a condition.

    Accepts a ``#rrggbb`` (or ``#rgb``) literal, returned lower-cased, or a
    palette name (``"red"``, ``"okabe_blue"``), returned as written for the
    theme to resolve. Checked once, here, so a typo surfaces when the
    configuration is read rather than halfway through drawing a figure.
    """
    ...
    if _HEX.match(text):
        return text.lower()
    if _PALETTE_NAME.match(text):
        return text
    raise KeyError(
        f"condition {name!r} was given colour {value!r}, which is neither a "
        "palette name (lower case, letters, digits and underscores, such as "
        "'okabe_blue') nor a #rrggbb value"
    )
...
class Condition:
    ...
    match: tuple[str, ...] = ()
    #: The declared colour, or None to auto-assign by declaration order.
...
class ConditionSet:
    ...
    #: flag travels into the manifest and is printed by every command that
    #: touches the design, because a fabricated group that reads as a real one
    #: is precisely the failure this module exists to stop.
    ...
    def colours(self) -> dict[str, str]:
        """Condition name to colour, auto-assigning a palette name where none was given.
        ...
        for entry in self.conditions:
            ...
            table[entry.name] = _AUTO_CYCLE[auto_index]
            auto_index += 1
        return table
    ...
    def as_dict(self) -> dict[str, Any]:
        """What goes in the run manifest's ``conditions`` section."""
