...
long an unbroken stretch of frames is. Ported unchanged from Motion's
``analysis/fields.py`` on 2026-09-21.
"""
...
__all__ = ["disc", "ground_within", "longest_run"]
...
def ground_within(ground: np.ndarray, radius: float) -> np.ndarray:
    ...
    One convolution answers it for every pixel at once. The result is clipped
    at zero: an FFT convolution of a binary mask returns values a few parts in
    1e-12 below zero where the true count is zero, and a negative area is not
    a thing.
    """
